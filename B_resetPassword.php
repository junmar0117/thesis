...
if(isset($_POST["password"]))
{
    ...
    if($pw === $cpw)
    {
        ...
        $query = mysqli_query($con, "UPDATE b_admin SET password='$pw' WHERE email = '$email'");
        if($query)
        {
    ...
    else
    {
        ...
        print '<script>window.location.assign("http://" . $_SERVER["HTTP_HOST"] . dirname($_SERVER["PHP_SELF"]) . "/B_resetPassword.php?code=$code".php");</script>'; // redirects to register.php
    }
}
...
<head>
    <meta charset = "utf-8">
    <title>AidPack | Barangay Change Password</title>
