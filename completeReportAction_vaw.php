...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    $alias = $_POST['alias'];
    $perpetrator_sex = $_POST['perpetrator_sex'];
    $perpetrator_date_of_birth = $_POST['perpetrator_date_of_birth'];
    $perpetrator_age = $_POST['perpetrator_age'];
    $perpetrator_civil_status = $_POST['perpetrator_civil_status'];
    $perpetrator_educational_attainment = $_POST['perpetrator_educational_attainment'];
    $perpetrator_nationality = $_POST['perpetrator_nationality'];
    $perpetrator_passport_no = $_POST['perpetrator_passport_no'];
    $perpetrator_occupation = $_POST['perpetrator_occupation'];
    $identifying_marks = $_POST['identifying_marks'];
    $perpetrator_religion = $_POST['perpetrator_religion'];
    $perpetrator_address = $_POST['perpetrator_address'];
    $perpetrator_region = $_POST['perpetrator_region'];
    $perpetrator_province = $_POST['perpetrator_province'];
    $perpetrator_city = $_POST['perpetrator_city'];
    $perpetrator_barangayy = $_POST['perpetrator_barangayy'];
    $relationship_perpetrator_to_victim = $_POST['relationship_perpetrator_to_victim'];
    ...
    if($bool)
    {
        ...
        mysqli_query($con, "INSERT INTO complete_report_vaw (username, reports_id, name,incident_time_date, place, barangay, description, file, type, incident, report_timesent, status, emergency, crime, latitude, longitude, handling_organization , address, region, province, city, barangayy, intake_by, position, case_manager, victim_name, sex, date_of_birth, age, civil_status, educational_attainment, nationality, passport_no, occupation, religion, victim_address, victim_region, victim_province, victim_city, victim_barangayy, with_disability, disability, no_of_children, ages_of_children, perpetrator_name, alias, perpetrator_sex, perpetrator_date_of_birth, perpetrator_age, perpetrator_civil_status, perpetrator_educational_attainment, perpetrator_nationality, perpetrator_passport_no, perpetrator_occupation, identifying_marks, perpetrator_religion, perpetrator_address, perpetrator_region, perpetrator_province, perpetrator_city, perpetrator_barangayy, relationship_perpetrator_to_victim, sexual_abuse, psychological, physical1, economic, others_incident, rape_sexual_intercourse, rape_sexual_assault, verbal, physical2, use_of_objects, child_prostitution, lascivious_conduct, anti_trafficking, anti_child_porn, anti_voyeurism, acts_of_lasciviousness, others_penal_code, description_incident, date_latest_incident, incident_address, incident_region, incident_province, incident_city, incident_barangayy, place_of_incident, witness_names, witness_address, witness_contact_no, lost_of_interest, reconciled, transfer_residence, lack_of_support, lack_of_confidence, others_case) VALUES ('$username','$id','$name','$when', '$place', '$barangay', '$description', '$file', '$type', '$incident', '$report_timesent', '$status', '$emergency', '$crime', '$latitude', '$longitude', '$handling_organization' , '$address', '$region', '$province', '$city', '$barangayy', '$intake_by', '$position', '$case_manager', '$victim_name', '$sex', '$date_of_birth', '$age', '$civil_status', '$educational_attainment', '$nationality', '$passport_no', '$occupation', '$religion', '$victim_address', '$victim_region', '$victim_province', '$victim_city', '$victim_barangayy','$with_disability','$disability','$no_of_children','$ages_of_children','$perpetrator_name', '$alias', '$perpetrator_sex', '$perpetrator_date_of_birth', '$perpetrator_age', '$perpetrator_civil_status', '$perpetrator_educational_attainment', '$perpetrator_nationality', '$perpetrator_passport_no', '$perpetrator_occupation', '$identifying_marks', '$perpetrator_religion', '$perpetrator_address', '$perpetrator_region', '$perpetrator_province', '$perpetrator_city', '$perpetrator_barangayy','$relationship_perpetrator_to_victim','$sexual_abuse','$psychological','$physical1','$economic', '$others_incident', '$rape_sexual_intercourse', '$rape_sexual_assault', '$verbal', '$physical2', '$use_of_objects', '$child_prostitution', '$lascivious_conduct', '$anti_trafficking', '$anti_child_porn', '$anti_voyeurism', '$acts_of_lasciviousness', '$others_penal_code', '$description_incident', '$date_latest_incident', '$incident_address', '$incident_region', '$incident_province', '$incident_city', '$incident_barangayy', '$place_of_incident','$witness_names','$witness_address','$witness_contact_no','$lost_of_interest','$reconciled', '$transfer_residence', '$lack_of_support', '$lack_of_confidence', '$others_case')"); //Inserts the value to table users
