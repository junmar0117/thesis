...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    if(isset($_POST['old_password']) && isset($_POST['new_password']) && isset($_POST['confirm_newpassword']))
    {
        ...
        if(empty($op))
        {
            header("Location: B_changePassword.php?error=Old Password is required");
            exit();
        }
        else if(empty($np))
        {
            header("Location: B_changePassword.php?error=New Password is required");
            exit();
        }
        else if($np !== $cnp)
        {
            header("Location: B_changePassword.php?error=Confirmation Password doesn't match");
            exit();
        }
        else
        {
            ...
            $query = "SELECT `password` FROM b_admin WHERE id = $id";
            ...
            if(mysqli_num_rows($results) === 1)
            {
                ...
                {  
                    ...
                    {
                        $query1 = "UPDATE b_admin SET `password` = '$np' WHERE id = '$id'";
                        $results1 = mysqli_query($con, $query1);
                        header("Location: B_changePassword.php?success=Password successfully updated");
                        exit();
                    }
                }
            }
            ...
            {
                header("Location: B_changePassword.php?error=Incorrect Password");
                exit();
            }
        }
    }
...
<head>
    <meta charset = "utf-8">
    <title> AidPack | Change Password</title>
...
<h3>Barangay Change Password</h3>
