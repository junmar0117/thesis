...
if($_SERVER['REQUEST_METHOD'] == "POST")
{
    ...
    if(isset($_POST['old_password']) && isset($_POST['new_password']) && isset($_POST['confirm_newpassword']))
    {
        ...
        if(empty($op))
        {
            header("Location: F_changePassword.php?error=Old Password is required");
            exit();
        }
        else if(empty($np))
        {
            header("Location: F_changePassword.php?error=New Password is required");
            exit();
        }
        else if($np !== $cnp)
        {
            header("Location: F_changePassword.php?error=Confirmation Password doesn't match");
            exit();
        }
        else
        {
            ...
            $query = "SELECT `password` FROM f_admin WHERE id = $id";
            ...
            if(mysqli_num_rows($results) === 1)
            {
                ...
                {  
                    ...
                    {
                        $query1 = "UPDATE f_admin SET `password` = '$np' WHERE id = '$id'";
                        $results1 = mysqli_query($con, $query1);
                        header("Location: F_changePassword.php?success=Password successfully updated");
                        exit();
                    }
                }
            }
            ...
            {
                header("Location: F_changePassword.php?error=Incorrect Password");
                exit();
            }
        }
    }
...
<head>
    <meta charset = "utf-8">
    <title> AidPack | BFP Change Password</title>
