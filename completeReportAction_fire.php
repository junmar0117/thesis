...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    $crime = $_POST['crime'];
    $when = $_POST['when'];
    $completed_by_admin_id = $_POST['f_id'];
    $featured = 0;
    ...
    if($bool)
    {
        mysqli_query($con, "INSERT INTO complete_reports (username, reports_id, name, incident_time_date, place, barangay, description, file, type, incident, report_timesent, status, emergency, crime, latitude, longitude, featured) VALUES ('$username','$id','$name','$when', '$place', '$barangay', '$description', '$file', '$type', '$incident', '$report_timesent', '$status', '$emergency', '$crime', '$latitude', '$longitude','$featured')"); //Inserts the value to table users

        mysqli_query($con, "INSERT INTO complete_report_fire (username, reports_id, name, incident_time_date, place, barangay, description, file, type, incident, report_timesent, status, emergency, crime, latitude, longitude, featured, building_address, building_number, fixed_property, incident_type, occupants_status, did_respond, called_via, respond_within_minutes, history, actions_recommendations, num_injuries, num_deaths, fire_origin, equipment, form_of_heat, type_material_ignited, form_of_material, method_of_extinguishment, level_of_fire, extent_flame, extent_smoke, detector_performance, sprinkler_performance, type_most_smoke, form_most_smoke, avenue_smoke_travel, mobile_property, year1, make1, model1, serial_number1, license_number, equipment_involved, year2, make2, model2, serial_number2, voltage, reporter, date_of_complete_report,completed_by_admin_id) VALUES ('$username','$id','$name','$when', '$place', '$barangay', '$description', '$file', '$type', '$incident', '$report_timesent', '$status', '$emergency', '$crime', '$latitude', '$longitude', '$featured', '$building_address' , '$building_number', '$fixed_property', '$incident_type', '$occupants_status', '$did_respond', '$called_via', '$respond_within_minutes', '$history', '$actions_recommendations', '$num_injuries', '$num_deaths', '$fire_origin', '$equipment', '$form_of_heat', '$type_material_ignited', '$form_of_material', '$method_of_extinguishment', '$level_of_fire', '$extent_flame', '$extent_smoke', '$detector_performance', '$sprinkler_performance', '$type_most_smoke','$form_most_smoke','$avenue_smoke_travel','$mobile_property','$year1','$make1', '$model1', '$serial_number1', '$license_number', '$equipment_involved', '$year2', '$make2', '$model2', '$serial_number2', '$voltage', '$reporter', '$date_of_complete_report','$completed_by_admin_id')"); //Inserts the value to table users
